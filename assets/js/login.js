...
/***/ function(module, exports, __webpack_require__) {
	...
	var _validate = __webpack_require__(505);
	...
	window.onload = function () {
	    _reactDom2.default.render(_react2.default.createElement(_login2.default, null), document.getElementById('l_login'));
	    MZ.validate.render({
	        wrap: $('#validate'),
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							...
							React.createElement(
								'span',
								{ className: 'd clearfix' },
								React.createElement('input', { type: 'text', name: 'account', value: this.state.account, onChange: this.changeHandle, placeholder: '账号' })
							),
							React.createElement('span', { className: 'em' })
						),
						React.createElement(
							'dd',
							{ className: 'clearfix' },
							React.createElement(
								'span',
								{ className: 'd clearfix' },
								React.createElement('input', { type: 'password', name: 'password', value: this.state.password, onChange: this.changeHandle, placeholder: '密码' })
							),
							React.createElement('span', { className: 'em' })
						),
						React.createElement(
							...
							React.createElement(
								'span',
								{ className: 'm clearfix' },
								React.createElement('input', { type: 'text', name: 'valicode', value: this.state.valicode, onChange: this.changeHandle, placeholder: '验证码' })
							),
							React.createElement(
								'span',
								{ className: 'm clearfix' },
								React.createElement('img', { src: this.state.valisrc })
							),
							React.createElement(
								'span',
								{ className: 'a' },
								React.createElement(
									'a',
									{ href: 'javascript:void(0);', className: 'l_change' },
									'换一张'
								)
							),
							React.createElement('span', { className: 'em mem' })
						)
					),
					React.createElement(
						'div',
					...
					React.createElement(
						'a',
						{ href: 'javascript:void(0);', className: 'l_submit' },
						'登录'
					)
				),
				React.createElement(
					...
					React.createElement(
						'a',
						{ href: 'javascript:void(0);', className: 'l_regsiter' },
						'我要注册'
					),
					React.createElement(
						'a',
						{ href: 'javascript:void(0);', className: 'l_forget' },
						'忘记密码？'
					)
				)
			);
...
/***/ function(module, exports) {
	...
	(function ($) {
	     ...
	     *@name MZ.validate
	     */
	    MZ.namespace("MZ.ui.validate", MZ.extend({
	        /**
	         *初始化入口，render
	         ...
	         *主要验证模块
	         */
	        _checkAction: function _checkAction(input) {
	            var o = input,
	                _self = this;
	        ...
	        _submitForm: function _submitForm() {
	            ...
	            if (_self.canSubmit) {
	                if (typeof _self.opt.callback == 'function') _self.opt.callback();
	            };
	        },
	        /**
	         *支撑函数，包括字符串长度计算、input类型判断、placeholder等
	         */
	        _supportFun: function _supportFun() {
	            this.checkLength = function (input) {
	                var len = 0;
	                var val = input[0].value;
	                var length = val.length;
	                for (var i = 0; i < length; i++) {
	                    if (val[i].match(/[^\x00-\xff]/ig) != null) len += 2;else len += 1;

	                    if (len / 2 > length) break;
	                }
	                if (len < 40) return true;else return false;
	            };
	            //判断是否为checkbox
	            this.isCheckbox = function (o) {
	                var checkbox = false;
	                if (o.obj[0].type == 'checkbox') checkbox = true;else checkbox = false;
	                return checkbox;
	            };
	            //判断是否为radio
